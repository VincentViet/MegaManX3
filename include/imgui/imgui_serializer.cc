...
#include "imgui.h"
#include "imgui_serializer.h"

// From <imgui.cpp>:--------------------------------------------------------
#include <cstdio>      // vsnprintf
#define IM_ARRAYSIZE(_ARR)          ((int)(sizeof(_ARR)/sizeof(*_ARR)))

static size_t ImFormatString(char* buf, const size_t buf_size, const char* fmt, ...)
{
	va_list args;
	va_start(args, fmt);
	const auto w = vsnprintf(buf, buf_size, fmt, args);
	va_end(args);
	buf[buf_size - 1] = 0;
	return (w == -1) ? buf_size : static_cast<size_t>(w);
}
//---------------------------------------------------------------------------
...
bool ImGui::SaveStyle(const char* filename, const ImGuiStyle& style, int style_color)
{
	// Write .style file
	FILE* f;
	...
	fprintf(f, "[DefaultStyleColor]\n%d\n", style_color);

	for (size_t i = 0; i != ImGuiCol_COUNT; i++)
	{
		const auto& c = style.Colors[i];
...
bool ImGui::LoadStyle(const char* filename, ImGuiStyle& style, int& style_color)
{
	// Load .style file
	...
	FILE* f;
	if (fopen_s(&f, filename, "rt")) return false;
	...
	auto f_size = static_cast<size_t>(f_size_signed);
	if (fseek(f, 0, SEEK_SET)) {
		fclose(f);
		return false;
	}
	const auto f_data = static_cast<char*>(ImGui::MemAlloc(f_size + 1));
	...
	for (const char* line_start = f_data; line_start < buf_end; )
	{
		...
		else if (name[0] != '\0')
		{
			...
			float* pf[4] = { nullptr,nullptr,nullptr,nullptr };
			auto npf = 0;
			...
			// parsing 'name' here by filling the fields above
			{
				...
				else if (strcmp(name, "WindowMinSize") == 0) { npf = 1; pf[0] = &style.WindowMinSize.x; pf[1] = &style.WindowMinSize.y; }
				...
				else if (strcmp(name, "FramePadding") == 0) { npf = 1; pf[0] = &style.FramePadding.x; pf[1] = &style.FramePadding.y; }
				...
				else if (strcmp(name, "DefaultStyleColor") == 0) { npf = 0; }
				// all the colors here
				else {
					for (auto j = 0; j < ImGuiCol_COUNT; j++) {
						if (strcmp(name, ImGui::GetStyleColorName(j)) == 0) {
							npf = 4;
							auto& color = style.Colors[j];
			...
			switch (npf) {
			case 0:
				if (sscanf_s(line_start, "%d", &style_color) == 1)
					break;
	...
	switch (style_color)
	{
	case 0: ImGui::StyleColorsClassic(&ImGui::GetStyle()); break;
	case 1: ImGui::StyleColorsDark(&ImGui::GetStyle()); break;
	case 2: ImGui::StyleColorsLight(&ImGui::GetStyle()); break;
	default: break;
	}
	return true;
}
